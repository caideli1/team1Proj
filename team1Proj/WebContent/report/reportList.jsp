<%@page import="com.team1.huangsai.dao.Tb_userDao"%>
...
function turnPage(pageSize){
   ...
   window.location="${pageContext.request.contextPath}/report?method=showReportList&pageNo=1&pageSize="+pageSize;
}
</script>
</head>
...
class="newfont03" align="center">
  <caption class="tablestyle_title">报修单管理-->报修单列表</caption>
  <tr class="firstRow">
   <td width="8%">编号</td>
   ...
   <td  width="10%">
    ...
    <td>
        <c:choose>
                         ...
                         <c:otherwise>
                               <a href="${pageContext.request.contextPath}/report?method=RepDetailes&id=${sta.count }&pageNo=${requestScope.page.pageNo}">维修详情</a>
                         </c:otherwise>
                       </c:choose>  
             ...
             <c:choose>
              ...
              <c:when test="${requestScope.page.pageSize==10}">
               ...
               <c:otherwise>
                 ...
                 <div class="showPage">
                 [<a href="${pageContext.request.contextPath }/report?method=showReportList&pageNo=1" class="right-font08">首页</a> |  
                   <c:choose>
                   <c:when test="${requestScope.page.pageNo>1 }">
                      <a href="${pageContext.request.contextPath }/report?method=showReportList&pageNo=${requestScope.page.pageNo-1}" class="right-font08">上一页</a> 
                   </c:when>
                  <c:otherwise>
                                                          上一页
                  </c:otherwise>
                ...
                <c:choose>
                   <c:when test="${requestScope.page.pageNo<requestScope.page.totalPage}">
                      <a href="${pageContext.request.contextPath }/report?method=showReportList&pageNo=${requestScope.page.pageNo+1}" class="right-font08">下一页</a> 
                   </c:when>
                  <c:otherwise>
                                                          下一页
                  </c:otherwise>
                </c:choose>   
                | <a href="${pageContext.request.contextPath }/report?method=showReportList&pageNo=${requestScope.page.totalPage}" class="right-font08">末页</a>]
                 </div>

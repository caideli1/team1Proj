...
function turnPage(pageSize){
   window.location="${pageContext.request.contextPath}/complain?method=doAllComplain&complainpageNo=1&pageSize="+pageSize;
}
</script>
</head>
...
<body>
 <table width="98%" border="0" cellpadding="3" cellspacing="1" bgcolor="#464646" class="newfont03" align="center">
  <caption class="tablestyle_title">投诉单管理-->投诉单列表</caption>
  <tr class="firstRow">
   <td width="8%">编号</td>
  	...
  	<c:forEach var="obj" items="${complainpage.data}">
		  ...
		  <tr class="cont">
		   ...
		   <c:choose>
		   	<c:when test="${obj[4]==0 }">
		   		<td>未受理</td>
			    <td><a href="${pageContext.request.contextPath }/complain?method=openAdd&complainId=${obj[0]}">分配处理人员</a></td>
		   	</c:when>
		   <c:when test="${obj[4]==1 }">
		   		<td>已受理</td>
		   ...
		   <c:when test="${obj[4]==2 }">
		   		<td>已处理</td>
		   		<td><a href="${pageContext.request.contextPath }/complain?method=doDetail&complainId=${obj[0]}">处理详情</a></td>
		   </c:when>
		   </c:choose>
		  </tr>
	</c:forEach>   
